...
= \argmax_{\alpha_x} \sum_t - (F_{x,t} - \alpha_x \hC_t - \hbeta_x)^2, \label{eq:alpha_x}
\end{align}
\end{subequations}
which is solved by regressing $\hbC$ onto $(\bF_x - \hbeta_x \ve{1}_T)$.  In other words, by computing the derivative of Eq.~\eqref{eq:alpha_x} with respect to $\alpha_x$ and setting to zero, one obtains (using Matlab notation): $\halpha_x = \hbC \backslash (\bF_x - \hbeta_x \ve{1}_T)$. % Because the denominator is \emph{not} a function of $x$, and the scale of $C_t$ relative to $F_t$ is arbitrary, this is equivalent to $\halpha_x=\frac{1}{T} \sum_t F_{x,t}$, i.e.,  the optimal spatial filter is obtained from the mean intensity of each pixel.  
...
\caption{stats} \label{fig:stats-data}
\end{figure}
