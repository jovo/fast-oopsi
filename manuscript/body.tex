...
\end{align}
which can be solved by computing the derivative of Eq.~\eqref{eq:alpha_x2} with respect to $\bX$ and setting to zero, or using Matlab notation: $\hbX = \hA \backslash \bF_x$. % Because the denominator is \emph{not} a function of $x$, and the scale of $C_t$ relative to $F_t$ is arbitrary, this is equivalent to $\halpha_x=\frac{1}{T} \sum_t F_{x,t}$, i.e.,  the optimal spatial filter is obtained from the mean intensity of each pixel.  
% Computing the derivative of Eq.~\eqref{eq:valpha_x} with respect to each $\hbeta_x$, setting the result to zero, and solving for $\hbeta_x$, yields:
% \begin{align} \label{eq:beta}
% 	\hbeta_x = \frac{1}{T} \sum_{t=1}^T \frac{F_{x,t} - \halpha_x  \hC_t}{\halpha_x^2}.
% \end{align} 
% Iterating these two steps results in a coordinate ascent approach to estimate $\valpha$ and $\vbeta$ \cite{CONV04}. 
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/fast_wiener_inf13}
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/stats}
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/fast_wiener_learn13}
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/fast_wiener_vitro4}
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/fast_wiener_burst12}
...
% \begin{figure}[h!]
% \centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/stats_data_bursts}
% \caption{stats} \label{fig:stats-data-bursts}
% \end{figure}

\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/stats_data}
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/fast_smc_vitro12}
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/joshyv/Research/oopsi/fast-oopsi/figs/spatial_background}
\caption[spatial filtering can improve effective SNR]{A simulation demonstrating that using a better spatial filter can significantly enhance the effective SNR. The true spatial filter was a difference of Gaussians: a positively weighted Gaussian of small width, and a negatively weighted Gaussian with larger width (both with the same center).  Top left: true spatial filter.  Top right: boxcar filter.  Middle left: fluorescence projection using true spatial filter. Middle right: fluorescence projection using boxcar filter.  Bottom left: fast filter inference when using true spatial filter.  Bottom right: fast filter inference when using boxcar spatial filter.  Simulation parameters: $\valpha=\mN(\ve{0},2 \bI)-0.5 \mN(\ve{0},2.5 \bI)$ where $\mN(\ve{\mu},\ve{\Sig})$ indicates a two-dimensional Gaussian with mean $\ve{\mu}$ and covariance matrix $\ve{\Sig}$, $\vbeta=\ve{0}$, $\tau=0.85$ sec, $\lam=5$ Hz.} \label{fig:spatial} 
\end{figure}
