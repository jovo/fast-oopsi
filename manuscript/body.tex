...
\hbth &= \argmax_{\bth} \int P[\bF, \bC | \bth] d\bC = \argmax_{\bth} \int P[\bF | \bC; \bth] d\bC \int P[\bn | \bth] d\bn.
\end{align}
...
\hbth &\approx \argmax_{\bth} P[\bF, \hbC; \bth] = \argmax_{\bth} P[\bF| \hbC; \bth] P[\hbn | \bth]
\end{align}
...
\end{align}
\noindent where $\alpha$ is not present because the fluorescence is arbitrarily scaled, and $\gam$ is not present because its posterior is relatively flat, ie, large changes in $\gam$ result in relatively small changes in the posterior. $\beta$ is estimated using:
\begin{align}
	\hbeta &= \argmax_{\beta >0} \prod_{t=1}^T P[F_t | \hC_t; \beta,\sig] = \argmax_{\beta>0} \sum_{t=1}^T (F_t - (C_t + \beta))^2,
\end{align}
\noindent which is solved by letting $\hbeta=\langle \bF-\bC \rangle_t$, where $\langle \cdot \rangle_t$ indicates a mean over $t$.  $\sig$ is then the root-mean-square of the residuals, and $\hlam$ is the mean of $\hbn$. 
...
In the above, the observation distribution was always assumed to be Gaussian.  The statistics of photon emission and counting, however, suggest that a Poisson distribution would be more natural \cite{SjulsonMiesenbock07}, yielding:
\begin{align} \label{eq:poiss}
	F_t \overset{iid}{\sim}\text{Poisson}(F_t; \alpha C_t + \beta).
\end{align}
...
In Eq. \eqref{eq:n}, the rate of spiking was assumed to be a constant.  Often, additional knowledge about the experiment, including external stimuli, or other neurons spiking, can provide strong time-varying prior information.  A simple model modification can incorporate that feature:
\begin{align}
