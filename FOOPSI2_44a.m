function [n P]=FOOPSI2_44a(F,P,Sim)
...
O       = 1+0*F(1:T-1);                         % init a unity vector
M       = spdiags([-P.gamma*O O]/P.rho, -1:0,T,T-1); % matrix transforming calcium into spikes, ie n=M*C
I       = speye(T-1);                           % create out here cuz it must be reused
Hmat1   = 2*c*I;                                % pre-compute matrix for hessian
Hmat2   = I;                                    % another one
col1    = 1:T-1;                                % 1st colomn of any TxT matrix (used for M)
diags   = 1:T:(T-1)^2;                          % index of diagonal elements of TxT matrices
offdiags= 2:T:(T-1)^2;                          % index of off-diagonal elements (the diagonal below the diagonal) of TxT matrices
M(col1) = -P.nu*P.rho;                          % for baseline
...
for i=1:Sim.MaxIter
    ...
    P       = FastParams2_6(F,C,n,T,dt,Q);      % update parameters
    ...
    function [n C] = FastFilter(F,P)
        ...
        C = filter(P.rho,[1, -P.gamma],n+P.nu/P.rho); % initialize calcium
        M(offdiags) = -P.gamma/P.rho;           % matrix transforming calcium into spikes, ie n=M*C
        M(diags)    = 1/P.rho;                  % update diags of M
        M(col1)     = -P.nu*P.rho;              % update 1st col of M
        Hmat1(diags)= 2*c;                      % for expediency
        sumM        = sum(M)';                  % for expediency
        
        while e>1e-13                           % this is an arbitrary threshold

            D = F(1:T-1)-C;                     % difference vector
