function [n_best P_best]=FOOPSI_v1_5(F,P,Sim)
...
for i=1:Sim.MaxIter
    ...
    eta = 1;                                % weight on barrier function
        n = o*(eta/P.lam);                  % initialize n
        C = filter(1,[1 -a],n);             % initialize C
...
%     P.lam = T/(Sim.dt*sum(n));
    ...
    if Sim.Plot == true
        subplot(311), hold on, plot(i,lik(i+1),'o'), axis('tight')
