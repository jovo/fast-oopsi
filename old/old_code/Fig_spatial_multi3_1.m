...
User.Plot    = 0;                           % whether to plot filter with each iteration
...
%             Neta.h  = 1;
%             Vser.Nc =1;
%             [J{j}.n J{j}.P] = FOOPSI_v3_02_03(G,PP,Neta,Vser);
%             I{q}.n(:,j)=J{j}.n;
%             I{q}.P.a(:,j)=P.a(:,j);
%             I{q}.P.b=P.b;
%             I{q}.P.gam(j)=J{j}.P.gam;
%             I{q}.P.lam(j)=J{j}.P.lam;
%         end
...
% Pl.xlim = [200 400]+300;        % limits of x-axis
...
%         plot(z1(F_proj)+(j-1)*0.5+1,'Color',Pl.c(j,:),'LineWidth',Pl.lw)
%         bar(Pl.n(:,j)*j/2,'FaceColor',Pl.c(j,:),'EdgeColor',Pl.c(j,:),'LineWidth',2)
%         axis([Pl.xlim 0 2.5])
%         set(gca,'XTick',Pl.XTick,'XTickLabel',[])
%         set(gca,'YTick',[0 1 2.5],'YTickLabel',[])
%         set(gca,'XTick',Pl.XTick,'XTickLabel',[])
...
%         bar(edges+diff(edges(1:2)/2),joint_hist(:,2),'FaceColor',Pl.g,'EdgeColor',Pl.g,'BarWidth',0.5)
% 
%         set(gca,'XTick',xticks,'XTickLabel',xticks,'YTickLabel',[])
%         axis('tight')
%         if q==1 && k==1
%             ylab=ylabel(['P(n_{MAP} | n=1)'],'FontSize',Pl.fs);
%             title('Independent Inference','FontSize',Pl.fs)
%         elseif q==1 && k==2
%             ylab=ylabel(['P(n_{MAP} | n=0)'],'FontSize',Pl.fs);
% 
%         elseif q==2 && k==1
%             title('Joint Inference','FontSize',Pl.fs)
%         end
%         set(ylab,'Rotation',0,'HorizontalAlignment','right','verticalalignment','middle')
%     end
% end
...
% % for q=1:length(I)
% %     for j=1:User.Nc
% %         subplot(nrows,ncols,j+(q-1)*ncols)
% %         bar(edges,D{q,j}.hist(:,1))
% %         set(gca,'XTick',[0 .5 1],'XTickLabel',[],'YTickLabel',[])
% %         axis('tight')
% % 
% %         subplot(nrows,ncols,j+nrows+(q-1)*ncols)
% %         bar(edges,D{q,j}.hist(:,2))
% %         set(gca,'XTickLabel',[0 .5 1],'XTickLabel',[],'YTickLabel',[])
% %         axis('tight')
% %     end
% % end
% 
% %% another bogus hist fig
% % figure(9),
% % for q=1:length(I)
% %     D{q}.sp=[];
% %     D{q}.no_sp=[];
