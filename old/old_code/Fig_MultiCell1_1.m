...
data_fname='/Users/joshyv/Research/projects/oopsi/fast-oopsi/';
...
mu      = [1 1]'*linspace(-1.5,1.5,Nc);         % means of gaussians for each cell (distributed across pixel space)
w       = Nc:-0.5:1;                          % weights of each filter
...
if GetROI==1;
    for i=1:Nc
        imagesc(reshape(sum(P.a,2),width,height))
        [x y]   = ginput(4);
        ROWS    = [round(mean(y(1:2))) round(mean(y(3:4)))];                              % define ROI
        COLS    = [round(mean(x([1 4]))) round(mean(x(2:3)))];
        COLS1{i}=COLS;
        ROWS1{i}=ROWS;
        save([data_fname 'ROIs'],'ROWS1','COLS1')
    end
elseif GetROI==2;
%         tif_name=[data_fname 'filters.tif'];
%         imwrite(reshape(sum(P.a,2),Sim.w,Sim.h),tif_name,'tif','Compression','none','WriteMode','overwrite')
        imagesc(reshape(sum(P.a,2),width,height))
%         imshow(tif_name);
        BW = roipoly;
        ind1=find(BW==1);
        ind0=find(BW==0);
        save([data_fname 'inds'],'ind0','ind1')
else
    load([data_fname 'inds'])
%     load('../../data/ROIs')
end
...
for q=qs%(qs>3)
    mn(q+2) = min([min(I{q}.P.a(:)) min(I{q}.P.b)]);
    mx(q+2) = (max([max(I{q}.P.a(:)) max(I{q}.P.b)])-mn(q))/60;
end
...
imagesc(reshape(sum(P.a,2),Sim.w,Sim.h)),
ylabel('true');
title('sum of filters')
subplot(ncols,nrows,1+nrows),
imagesc(reshape(sum(I{q}.P.a,2),Sim.w,Sim.h)),
ylabel('estimated'); 
...
for i=1:Nc, % true filters
    ...
    subplot(ncols,nrows,i+1+nrows),
    imagesc(reshape((I{q}.P.a(:,i)-mn(2+q))/mx(2+q),Sim.w,Sim.h)),
end
subplot(ncols,nrows,nrows), image(reshape((P.b-mn(1)/mx(1)),Sim.w,Sim.h)), title('background')
subplot(ncols,nrows,nrows*ncols), imagesc(reshape((I{q}.P.b-mn(2+q))/mx(2+q),Sim.w,Sim.h))
...
wh=[7 5];   %width and height
set(fnum,'PaperPosition',[0 11-wh(2) wh]);
print('-depsc','../../docs/journal_paper/figs/est_filers')

% % plot iterative lik
% for q=qs(qs>3)
% q=1;
fnum=fnum+1; figure(fnum), clf
subplot(211), plot(I{q}.P.l), axis('tight'), ylabel('lik')
subplot(212), plot(diff(I{q}.P.l)), ylabel('diff(lik'), hold on, plot(0*diff(I{q}.P.l),'k'), hold off, axis('tight')
% print fig
wh=[7 5];   %width and height
set(fnum,'PaperPosition',[0 11-wh(2) wh]);
...
% set(fnum,'PaperPosition',[0 11-wh(2) wh]);
% print('-depsc','../../docs/journal_paper/figs/filters')
