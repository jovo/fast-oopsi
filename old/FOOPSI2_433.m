function [n P]=FOOPSI2_433(F,P,Sim)
...
if ~isfield(Sim,'MaxIter') || Sim.MaxIter==0,
    ...
    l   = zeros(1,Sim.MaxIter);                 % extize likelihood
    ...
    if DoPlot == 1
        ...
        fprintf('sig=%.2f, gam=%.2f, nu=%.2f, rho=%.2f, lam=%.2f, lik=%.2g\n',...
            P.sig, P.gam, P.nu, P.rho, P.lam, l(1))
    end
end
...
for i=1:Sim.MaxIter
    ...
    P       = FastParams2_81(F,C,n,T,dt,Q);      % update parameters
    ...
    if DoPlot == 1
        ...
        fprintf('sig=%.2f, gam=%.2f, nu=%.2f, rho=%.2f, lam=%.2f, lik=%.2g\n',...
            P.sig, P.gam, P.nu, P.rho, P.lam, l(i+1))
    end
