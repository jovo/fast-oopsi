function [n_best P_best]=FOOPSI_v1_2(F,P,Sim)
...
%     C=filter(1,[1 -(1-Sim.dt/P.tau)],n);
    ...
    if Sim.Mstep==1;
        lik(i)     = L; %c*(F-C)'*(F-C)+P.lam*sum(n);
        if i<=Sim.MaxIter % lik(i)-lik(i-1)<=-1e-4
              if Sim.Plot == true
                  subplot(311), hold on, plot(i,lik(i),'o'), axis('tight')
                  subplot(312), cla, hold on, plot(F,'.k'), plot(C,'b'), drawnow
                  subplot(313), cla, plot(F,'k'), hold on, plot(C), bar(n,'EdgeColor','r','FaceColor','r'), hold off
...
%             Hmat  = 2*c*Tmat;
            ...
            P.lam = T/(Sim.dt*sum(n));          % update lambda

% fprintf('tau=%.2f, sig=%.2f, lam=%.2f, L=%.2f\n',P.tau,P.sig,P.lam,lik(i-1))
% figure(2), clf, plot(F,'k'), hold on, plot(C), bar(n), hold off
        else
            conv = true;
        end
    else
