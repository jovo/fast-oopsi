function [n_best P_best]=FOOPSI2_1(F,P,Sim)
...
P.a     = 1 - dt/P.tau;                     % decay factor
P.c     = 1/(2*P.sig^2);                    % scale of variance

% define some stuff for speed
o       = 1+0*F;                            % init a unity vector
M       = spdiags([-P.a*o o], -1:0,T,T);      % matrix transforming calcium into spikes, ie n=M*C
I       = speye(T);                         % create out here cuz it must be reused
Hmat1   = 2*P.c*I;                            % pre-compute matrix for hessian
Hmat2   = I;                                % another one
diags   = 1:T+1:T^2;
...
DD      = (F-C-P.mu)'*(F-C-P.mu);           % squared error
lik     = zeros(1,Sim.MaxIter);             % extize likelihood
lik(1)  = .5*T*log(2*pi*P.sig^2) + P.c*DD - T*log(P.lam*dt) + P.lam*dt*sum(n);% initialize likelihood
...
P_best.i = i;
    ...
    function [n C] = FastFilter(F,P)
        ...
        n   = o*(eta/P.lam);                    % initial n
        C   = filter(1,[1, -P.a],n);            % initial C
        M   = spdiags([-P.a*o o], -1:0,T,T);      % matrix transforming calcium into spikes, ie n=M*C

        while eta>1e-13                         % this is an arbitrary threshold

            D = F-C-P.mu;                       % difference vector
            L = P.c*D'*D+P.lam*dt*sum(n)-eta*sum(log(n));  % Likilihood function using C
            s = 1;                              % step size
            d = 1;                              % direction
            while norm(d)>5e-2 && s > 1e-3      % converge for this eta (again, these thresholds are arbitrary)
                g   = -2*P.c*D + P.lam*dt*sum(M)' - eta*M'*(n.^-1);  % gradient
                Hmat2(diags) = n.^-2;
                ...
                while L_new>=L+1e-7             % make sure newton step doesn't increase objective
                    ...
                    n       = M*C_new;
                    D       = F-C_new-P.mu;
                    L_new   = P.c*D'*D+P.lam*dt*sum(n)-eta*sum(log(n));
