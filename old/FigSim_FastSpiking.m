...
Sim.MaxIter = 1000;            % max number of iterations
Sim.Mstep   = 1;

% 2) initialize parameters
P.lam   = 10;               % rate, ie, lam*dt=# spikes per second                   
P.tau   = 0.9;              % calcium decay time constant (sec)
...
Phat = FastParams1_1(F,C,n,Sim.T,Sim.dt);
...
% P2.mu  = 1*P.mu;
P2.sig = P.sig;
Algs=[23]; %[14 17 18 19 20];                                       % which algorithms within DataComp to use
for m=Algs
    Sim.Alg = m; 
    tic
    I{m}    = DataComp14(F,P2,Sim);
    toc
end
%% 6) plot results
