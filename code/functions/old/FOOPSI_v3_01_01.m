function [n_best P_best]=FOOPSI_v3_01_01(F,P,Sim)
...
for i=2:Sim.MaxIter
    ...
    if abs((l(i)-l(i-1))/l(i))<1e-5 break; end         % if lik doesn't change, stop iterating

    if Sim.plot == 1
        figure(400), nrows=1+Nc;
        for j=1:Nc, subplot(1,nrows,j),
            imagesc(reshape(z1(P.a(:,j)),Sim.h,Sim.w)),
            title('a')
        end
        subplot(1,nrows,nrows), imagesc(reshape(z1(P.b),Sim.h,Sim.w)), title('b')
