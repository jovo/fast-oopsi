...
% 2_44: addes calcium baseline to the mix
%%
fprintf('\nFOOPSI2_44\n')
...
M   = spdiags([O -P.gam*O]/P.rho, 0:1,T-1,T-1); % matrix transforming calcium into spikes, ie n=M*C
I   = speye(T-1);                               % create out here cuz it must be reused
H1  = 2*c*I;                                    % pre-compute matrix for hessian
H2  = I;                                        % another one
d0  = 1:T:(T-1)^2;                              % index of diagonal elements of TxT matrices
d1  = T:T:(T-1)^2;                              % index of off-diagonal elements (the diagonal below the diagonal) of TxT matrices
...
for i=1:Sim.MaxIter
    ...
    function [n C] = FastFilter(F,P)
        ...
        C       = filter(P.rho,[1, -P.gam],n+P.nu/P.rho); % initialize calcium
        M(d1)   = -P.gam/P.rho;                 % matrix transforming calcium into spikes, ie n=M*C
        M(d0)   = 1/P.rho;                      % update d0 of M
