...
%
% F_t = alpha*C_t + beta + sig*eps_t, eps_t ~ N(0,1)
% C_t = gamma C_{t-1} + nu + rho*n_t,   n_t ~ Poisson(n_t; lambda*dt)
%
...
% 2_5: reparameterized for stability.  uses constrained optimization.
%%
fprintf('\nFOOPSI2_5\n')
...
M       = spdiags([-P.gamma*O O], -1:0,T,T);% matrix transforming calcium into spikes, ie n=M*C
...
if ~isfield(Sim,'MaxIter') || Sim.MaxIter==0,
    ...
    DD      = (F-P.alpha*C-P.beta)'*(F-P.alpha*C-P.beta);                     % squared error
    lik     = zeros(1,Sim.MaxIter);             % extize likelihood
    lik(1)  = 0.5*T*log(2*pi*P.sig^2) + DD/(2*P.sig^2) - T*log(P.lam*dt) + P.lam*dt*sum(n);% initialize likelihood
...
P_best.i = i;
    ...
    function [n C] = FastFilter(F,P)
        ...
        M(offdiags) = -P.gamma;                         % matrix transforming calcium into spikes, ie n=M*C
        Hmat1(diags)= 2*c;                              % pre-compute matrix for hessian
        sumM        = sum(M)';                          % pre-compute for expediency
