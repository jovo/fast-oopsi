...
P.sig   = 0.1;                                 % stan dev of noise (indep for each pixel)
...
figure(1), clf
subplot(221),imagesc(reshape(P.a,Sim.w,Sim.h))
subplot(222),imagesc(reshape(mean(F),Sim.w,Sim.h)), title(num2str(norm(P.a/norm(P.a)-mean(F)'/norm(mean(F)))))
subplot(223),imagesc(reshape(var(F),Sim.w,Sim.h)), title(num2str(norm(P.a/norm(P.a)-var(F)'/norm(var(F)))))
subplot(224),imagesc(reshape(mean(F)./var(F),Sim.w,Sim.h)), title(num2str(norm(P.a/norm(P.a)-(mean(F)./var(F))'/norm(mean(F)./var(F)))))
