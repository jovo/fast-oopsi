...
lam   = [.01 .05 .1 .5 1 5 10 50 100]; %sin(linspace(0,pi,dim))';
...
%         n(n>1)  = 1;
        ...
        I{3,q}.n = I{2,q}.n; 
        [ind foo]=find(I{2,q}.n<0);
        I{3,q}.n(ind)=0;
        I{4,q}.n = [diff(F); 0];

        for i=1:4
            snr{q}(tt,i)=mean(I{i,q}.n(D{tt,q}.n==1))/sqrt(mean(I{i,q}.n(D{tt,q}.n==0)/2.^2));
            
            if ~any(i==[2 4]) && ~isreal(snr{q}), keyboard; end
        end
    end
...
set(h,'LineWidth',2)
ymax=100;%max(mean_snr(:)+std_snr(:));
ymin=max(10^-1,min(mean_snr(:)-lb(:)));
axis([.9 length(lam)+.1 ymin ymax*1.1])
% set(gca,'YTick',logspace(log(ymin),log(ymax),5),'YTickLabel',0:5)
set(gca,'XTick',1:length(lam),'XTickLabel',lam,'FontSize',Pl.fs)
