...
tau     = round(100*rand(V.Ncells,1))/100+0.05;   % decay time constant for each cell
P.gam   = 1-V.dt./tau(1:V.Ncells);
P.lam   = 5*ones(Nc,1);                        % rate-ish, ie, lam*dt=# spikes per second
...
imagesc(reshape(mean(F,1),V.w,V.h))
