...
w       = 50*ones(Nc,1); %Nc:-1:1;                          % weights of each filter
for i=1:Nc
...
P.b     = 5;                              % baseline is a scaled down version of the sum of the spatial filters
...
P.lam   = linspace(5,15,Nc).*ones(Nc,1);                     % rate-ish, ie, lam*dt=# spikes per second
...
Z = 0*n(:,1);
F = poissrnd((C+P.b)*P.a');                  % fluorescence
